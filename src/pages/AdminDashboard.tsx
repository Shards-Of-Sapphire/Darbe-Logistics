...
import { FileCheck, FileText, Receipt, Database, UserCircle } from 'lucide-react';
...
type ActiveTab = 'acknowledgment' | 'consignor_note' | 'invoice' | 'database' | 'users';

export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<ActiveTab>('acknowledgment');
  const [editingAcknowledgment, setEditingAcknowledgment] = useState<AcknowledgmentEntry | undefined>();
  const [editingConsignorNote, setEditingConsignorNote] = useState<ConsignorNoteEntry | undefined>();
  const [editingInvoice, setEditingInvoice] = useState<InvoiceEntry | undefined>();

  const handleEdit = (type: string, data: any) => {
    if (type === 'acknowledgments') {
      setEditingAcknowledgment(data);
      setActiveTab('acknowledgment');
    } else if (type === 'consignor_notes') {
      setEditingConsignorNote(data);
      setActiveTab('consignor_note');
  ...
  return (
    <div className="w-full px-4 sm:px-6 md:px-8 py-6 sm:py-8">
      <div className="mb-6 sm:mb-8 no-print">
        <h1 className="text-xl sm:text-2xl font-black text-gray-900 uppercase tracking-tight">Admin Dashboard</h1>
        <p className="text-sm text-gray-500 italic">Manage your logistics operations and documentation.</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 sm:gap-8">
        ...
        <div className="flex-1 bg-white rounded-3xl shadow-sm border border-gray-50 p-4 sm:p-6 lg:p-8 min-h-[600px]">
          <div className={cn(activeTab === 'acknowledgment' ? 'block' : 'hidden')}>
            <AcknowledgmentForm initialData={editingAcknowledgment} onClear={() => setEditingAcknowledgment(undefined)} />
          </div>
          <div className={cn(activeTab === 'consignor_note' ? 'block' : 'hidden')}>
            <ConsignorNoteForm initialData={editingConsignorNote} onClear={() => setEditingConsignorNote(undefined)} />
          </div>
          <div className={cn(activeTab === 'invoice' ? 'block' : 'hidden')}>
            <InvoiceGenerator initialData={editingInvoice} onClear={() => setEditingInvoice(undefined)} />
          </div>
          <div className={cn(activeTab === 'database' ? 'block' : 'hidden')}>
            <DatabaseView onEdit={handleEdit} />
          </div>
